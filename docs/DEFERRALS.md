...
| D-07 | **Category / unbranded probe questions** — probe templates currently NAME the brand ("range of the Rivian R1S"), so Citation is trivially 100 (AI just echoes the name). Real citation needs category questions ("best electric SUV for towing?") measuring *unprompted* brand mentions vs competitors. Discovered in Sprint 4b verification | Interim: branded questions only; Citation over-optimistic | Before Citation score is trusted | 🔵 |
| D-08 | **Perplexity for earned-vs-owned** — ChatGPT/Claude (no browsing) return no citations, so earned_owned is null. Need Perplexity (or browsing models) to populate cited_sources | Interim: earned_owned only when Perplexity keyed | When first-party-vs-third-party citation share matters | 🔵 |
